...
use crate::{CompressedPublicKey, PublicKey};
...
impl<'a> FheTryEncrypt<EncryptableString<'a>, PublicKey> for FheAsciiString {
    type Error = crate::Error;

    fn try_encrypt(value: EncryptableString<'a>, key: &PublicKey) -> Result<Self, Self::Error> {
        let (str, padding) = value.str_and_padding();
        if !str.is_ascii() || str.contains('\0') {
            return Err(crate::Error::new(
                "Input is not an ASCII string".to_string(),
        ...
        let inner = crate::strings::public_key::PublicKey::new(&key.key).encrypt_ascii(str, padding);
        Ok(Self {
            inner: inner.into(),
            tag: key.tag.clone(),
            re_randomization_metadata: ReRandomizationMetadata::default(),
...
impl FheTryEncrypt<&str, PublicKey> for FheAsciiString {
    type Error = crate::Error;

    fn try_encrypt(value: &str, key: &PublicKey) -> Result<Self, Self::Error> {
        Self::try_encrypt(EncryptableString::NoPadding(value), key)
    }
}
