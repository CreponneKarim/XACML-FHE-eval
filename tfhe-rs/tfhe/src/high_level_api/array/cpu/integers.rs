...
use crate::keys::InternalServerKey;
use crate::prelude::{FheDecrypt, FheTryEncrypt, FheTryTrivialEncrypt};
use crate::{ClientKey, Error,PublicKey};
use rayon::prelude::*;
...
impl<'a, Clear, Id> FheTryEncrypt<&'a [Clear], PublicKey> for FheArrayBase<Vec<RadixCiphertext>, Id>
where
    Id: FheUintId,
    Clear: DecomposableInto<u64> + UnsignedNumeric,
{
    ...
    fn try_encrypt(clears: &'a [Clear], key: &PublicKey) -> Result<Self, Self::Error> {
        let num_blocks = Id::num_blocks(key.message_modulus());
        Ok(Self::new(
            clears
                .iter()
                .copied()
                .map(|clear| key.key.encrypt_radix(clear, num_blocks))
                .collect::<Vec<_>>(),
            vec![clears.len()],
        ))
    }
...
impl<'a, Clear, Id> FheTryTrivialEncrypt<&'a [Clear]> for FheArrayBase<Vec<RadixCiphertext>, Id>
where
    Id: FheUintId,
    Clear: DecomposableInto<u64> + UnsignedNumeric,
{
    ...
    fn try_encrypt_trivial(value: &'a [Clear]) -> Result<Self, Self::Error> {
        global_state::with_internal_keys(|key| match key {
            InternalServerKey::Cpu(key) => {
                Ok(Self::new(
                    ...
                    .map(|clear|key
                        .pbs_key()
                        .create_trivial_radix(clear, Id::num_blocks(key.message_modulus()))
                    ).collect::<Vec<_>>(),
                    vec![value.len()],
                ))
            }
