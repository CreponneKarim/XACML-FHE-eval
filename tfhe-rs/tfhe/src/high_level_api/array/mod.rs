...
///	added by Karim Laouchedi
pub fn fhe_base_radix_ciphertext_array_eq(lhs: &[BaseRadixCiphertext<Ciphertext>], rhs: &[BaseRadixCiphertext<Ciphertext>]) -> FheBool {
	global_state::with_internal_keys(|sks| match sks {
        InternalServerKey::Cpu(cpu_key) => {

            let result = cpu_key
                .pbs_key()
                .all_eq_slices_parallelized(&lhs, &rhs);
            FheBool::new(
                result,
                cpu_key.tag.clone(),
